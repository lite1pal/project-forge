...
export function createResourceClient(apiClient: ApiClient) {
  return {
    async create(organizationId: string, body: Record<string, unknown>) {
      return customerRecordSchema.parse(
        await apiClient.request({
          body,
          method: "POST",
          path: "/api/v1/organizations/:organizationId/customers/${organizationId}".replace("//", "/") as never
        })
      );
    },
    async get(organizationId: string, id: string) {
      return customerRecordSchema.parse(
        await apiClient.request({
          path: "/api/v1/organizations/:organizationId/customers/${organizationId}/${id}".replace("//", "/") as never
        })
      );
    },
